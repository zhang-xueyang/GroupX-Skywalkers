...
final_data <- dplyr::select(filtered_data, YEAR, SERIAL,AGE,SEX,MARITAL, EDUCATION, EMPLOYMENT, WORK_HOURS, WORK_HOURS_CAT,
HEALTH_STATUS, SADNESS,WORRY_FREQ, WORRY_LEVEL,DEPRESS_FREQ,DEPRESS_LEVEL,SAMPWEIGHT, STRATA,PSU)
...
cat("Female group:", n_female, "\n")
cat("Male group:", n_male, "\n")
cat("Total:", n_female + n_male, "\n")
...
cat("\nNumber of observations:\n")
cat("Female group:", n_female_dep, "\n")
cat("Male group:", n_male_dep, "\n")
cat("Total:", n_female_dep + n_male_dep, "\n")
...
print("Distribution of observations by education level:")
...
print("New group sizes:")
print(table(regression_data$education_binary))
...
cat("\nNumber of observations:\n")
cat("Lower education group:", n_low, "\n")
cat("Higher education group:", n_high, "\n")
cat("Total:", n_low + n_high, "\n")
...
cat("\nNumber of observations:\n")
cat("Lower education group:", n_low_dep, "\n")
cat("Higher education group:", n_high_dep, "\n")
cat("Total:", n_low_dep + n_high_dep, "\n")
...
print(knitr::kable(results_df, 
                   caption = "Sobel Test Results for Health Status Mediation",
                   digits = 3))

# Additional diagnostic information
cat("\nModel Summaries:\n")
cat("\nPath a (Work Hours -> Health):\n")
print(summary(model_health)$coefficients[treatment_levels, ])

cat("\nPath b (Health -> Sadness):\n")
print(summary(model_sadness_full)$coefficients["HEALTH_STATUS", ])
...
# Function to perform Sobel test for logistic outcome remains the same
sobel_test_logistic <- function(model_m, model_y, treatment_levels) {
  results <- list()
  
  for(level in treatment_levels) {
    # Get path a coefficient and SE (work hours -> health)
    a <- coef(model_m)[level]
    se_a <- sqrt(diag(vcov(model_m)))[level]
    
    # Get path b coefficient and SE (health -> depression)
    b <- coef(model_y)["HEALTH_STATUS"]
    se_b <- sqrt(diag(vcov(model_y)))["HEALTH_STATUS"]
...
# Path b and c': Depression model with health status
...
# Perform Sobel test
sobel_results_depression <- sobel_test_logistic(
  model_health, 
  model_depression_full, 
  treatment_levels
)

# Create results table with more decimal places
results_df_depression <- do.call(rbind, lapply(names(sobel_results_depression), function(level) {
  res <- sobel_results_depression[[level]]
  data.frame(
    Work_Hours_Category = level,
    Indirect_Effect = format(res$indirect_effect, digits = 10, scientific = FALSE),
    Standard_Error = format(res$se, digits = 10, scientific = FALSE),
    Z_Score = format(res$z_score, digits = 10, scientific = FALSE),
    P_Value = format(res$p_value, digits = 10, scientific = FALSE),
    row.names = NULL
  )
}))

# Calculate odds ratios with more decimal places
results_df_depression$Odds_Ratio_Indirect <- format(
  exp(as.numeric(results_df_depression$Indirect_Effect)), 
  digits = 10, 
  scientific = FALSE
)

# Print detailed results
print(knitr::kable(results_df_depression, 
                   caption = "Complete Mediation Results for Depression Model (Including Odds Ratios)",
                   align = c('l', 'r', 'r', 'r', 'r', 'r')))

# Print path coefficients with more detail
cat("\nDetailed Path Coefficients:\n")
cat("\nPath a (Work Hours -> Health):\n")
print(data.frame(
  Coefficient = format(coef(model_health)[treatment_levels], digits = 10, scientific = FALSE),
  SE = format(sqrt(diag(vcov(model_health)))[treatment_levels], digits = 10, scientific = FALSE)
))

cat("\nPath b (Health -> Depression):\n")
print(data.frame(
  Coefficient = format(coef(model_depression_full)["HEALTH_STATUS"], digits = 10, scientific = FALSE),
  SE = format(sqrt(diag(vcov(model_depression_full)))["HEALTH_STATUS"], digits = 10, scientific = FALSE)
))
